...
import { FirebaseAnalyticsProvider } from './firebase.provider.js';
...
describe('FirebaseAnalyticsProvider', () => {
  ...
  describe('initialization', () => {
    ...
    it('should throw error when Firebase is not available', async () => {
      ...
      await expect(provider.initialize({})).rejects.toThrow('Firebase not found');
    });

    it('should throw error when Firebase Analytics is not available', async () => {
      (window as any).firebase = {};

      await expect(provider.initialize({})).rejects.toThrow('Firebase Analytics not found');
    });
  });
  ...
  describe('trackEvent', () => {
    ...
    it('should track event with sanitized name', async () => {
      ...
      expect(mockAnalytics.logEvent).toHaveBeenCalledWith('test_event_', { value: 100 });
    });
    ...
    it('should sanitize long event names', async () => {
      ...
      expect(mockAnalytics.logEvent).toHaveBeenCalledWith('a'.repeat(40), {});
    });
  ...
  describe('logRevenue', () => {
    ...
    it('should log earn_virtual_currency for non-product revenue', async () => {
      ...
      expect(mockAnalytics.logEvent).toHaveBeenCalledWith('earn_virtual_currency', {
        value: 50,
        currency: 'USD',
      });
    });
