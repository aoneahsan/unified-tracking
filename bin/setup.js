...
class UnifiedTrackingSetup {
  ...
  async selectFromList(title, options, allowMultiple = false) {
    ...
    optionKeys.forEach((key, index) => {
      ...
      console.log(`${colors.dim}${index + 1}.${colors.reset} ${option.name} - ${colors.dim}${option.description}${colors.reset}`);
    });
    
    if (allowMultiple) {
      console.log(`${colors.dim}Enter numbers separated by commas (e.g., 1,3,5) or press Enter to skip:${colors.reset}`);
    } else {
    ...
    if (allowMultiple) {
      ...
      return selections
        .filter(i => i >= 0 && i < optionKeys.length)
        .map(i => optionKeys[i]);
    } else {
  ...
  async run() {
    try {
      ...
      if (enableConsent) {
        ...
        this.config.consent.defaultConsent.errorTracking = await this.confirmPrompt('Allow error tracking by default?', true);
        this.config.consent.defaultConsent.marketing = await this.confirmPrompt('Allow marketing by default?', false);
        this.config.consent.defaultConsent.personalization = await this.confirmPrompt('Allow personalization by default?', false);
      }
